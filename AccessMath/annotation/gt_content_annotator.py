...
class GTContentAnnotator(Screen):
    ...
    def __init__(self, size, video_metadadata, db_name, lecture_title, output_prefix, forced_resolution=None):
        ...
        # main video player
        player_type = ScreenVideoPlayer.VideoPlayerImageList if sequence_mode else ScreenVideoPlayer.VideoPlayerOpenCV

        self.player = ScreenVideoPlayer("video_player", 960, 540)
        self.player.position = (50, 100)
        self.player.open_video_files(self.video_files, forced_resolution, player_type, img_format)
        ...
        btn_object_add_bbox = ScreenButton("btn_object_add_bbox", "+ BBOX", 22, 100)
        btn_object_add_bbox.set_colors((192, 255, 128), (64, 64, 64))
        btn_object_add_bbox.position = (self.container_object_options.width - btn_object_add_bbox.width - 5, self.object_selector.get_top())
        btn_object_add_bbox.click_callback = self.btn_object_add_bbox
        self.container_object_options.append(btn_object_add_bbox)
        # ...... add a quadrilateral (polygon, sides 4)
        btn_object_add_quad = ScreenButton("btn_object_add_quad", "+ QUAD", 22, 100)
        btn_object_add_quad.set_colors((192, 255, 128), (64, 64, 64))
        btn_object_add_quad.position = (self.container_object_options.width - btn_object_add_quad.width - 5, btn_object_add_bbox.get_bottom() + 15)
        btn_object_add_quad.click_callback = self.btn_object_add_quad
        self.container_object_options.append(btn_object_add_quad)

        # ...... rename ....
        btn_object_rename = ScreenButton("btn_object_rename", "Rename", 22, 100)
        btn_object_rename.set_colors((192, 255, 128), (64, 64, 64))
        btn_object_rename.position = (self.container_object_options.width - btn_object_rename.width - 5, btn_object_add_quad.get_bottom() + 15)
        ...
        # ... key-frame buttons ...
        self.container_keyframe_options = ScreenContainer("container_keyframe_options", (425, 150), general_background)
        ...
        self.lbl_keyframe_label.position = (self.btn_keyframe_invisible.get_left(), self.lbl_keyframe_next.get_bottom() + 20)
        self.lbl_keyframe_label.set_color((255, 255, 255))
        self.lbl_keyframe_label.set_background(general_background)
        ...
        # self.txt_keyframe_label.position = (self.lbl_keyframe_label.get_right() + 10, self.lbl_keyframe_next.get_bottom() + 15)
        ...
        self.container_text_input = ScreenContainer("container_text_input", (425, 230), general_background)
        ...
        self.container_segment_labels = ScreenContainer("container_segment_labels", (425, 280), general_background)
        self.container_segment_labels.position = (self.canvas.get_right() + 5, self.container_text_input.get_bottom() + 10)
        ...
        save_button.position = (exit_button.get_left() - save_button.width - 20, self.height - save_button.height - 15)
        ...
        export_button.position = (save_button.get_left() - export_button.width - 20, self.height - export_button.height - 15)
        export_button.click_callback = self.btn_export_segments_click
        self.elements.append(export_button)

        # EXPORT Object Button
        export_object_button = ScreenButton("export_object_button", "EXPORT OBJECT", 16, 150, 0)
        export_object_button.set_colors((192, 255, 128), (64, 64, 64))
        export_object_button.position = (export_button.get_left() - export_object_button.width - 20, self.height - export_object_button.height - 15)
        export_object_button.click_callback = self.btn_export_object_click
        self.elements.append(export_object_button)

        # REDO Button
        redo_button = ScreenButton("redo_button", "REDO", 16, 100, 0)
        redo_button.set_colors((192, 255, 128), (64, 64, 64))
        redo_button.position = (export_object_button.get_left() - redo_button.width - 50, self.height - redo_button.height - 15)
        ...
        undo_button.position = (redo_button.get_left() - redo_button.width - 20, self.height - undo_button.height - 15)
    ...
    def add_object(self, id, name, frame, abs_time, shape_type, polygon_points):
        ...
        # add to canvas ....
        if self.video_objects[id].shape_type == VideoObject.ShapeAlignedRectangle:
            x, y = polygon_points[0]
            # right bottom  - left top
            w, h = polygon_points[2] - polygon_points[0]
            self.canvas.add_rectangle_element(id, x, y, w, h)
        elif self.video_objects[id].shape_type == VideoObject.ShapeQuadrilateral:
        ...
        for object_name in self.video_objects:
            ...
            if loc is None:
                if shape  == VideoObject.ShapeAlignedRectangle:
                    self.canvas.update_rectangle_element(object_name, 0, 0, 0, 0, False)
                elif shape == VideoObject.ShapeQuadrilateral:
                    self.canvas.update_polygon_element(object_name, None, False)
                else:
                    raise Exception("Uknown Video Object Shape")
            else:
                if shape == VideoObject.ShapeAlignedRectangle:
                    x, y = loc.polygon_points[0]
                    w, h = loc.polygon_points[2] - loc.polygon_points[0]
                    self.canvas.update_rectangle_element(object_name, x, y, w, h, loc.visible)
                elif shape == VideoObject.ShapeQuadrilateral:
                    self.canvas.update_polygon_element(object_name, loc.polygon_points, loc.visible)
    ...
    def canvas_object_edited(self, canvas, object_name):
        ...
        if self.video_objects[object_name].shape_type == VideoObject.ShapeAlignedRectangle:
        ...
        elif self.video_objects[object_name].shape_type == VideoObject.ShapeQuadrilateral:
    ...
    def prepare_confirm_input_mode(self, text_operation, textbox_text, message_text):
        ...
        self.label_txt_object_name.visible = (text_operation == 1 or text_operation == 2 or text_operation == 5)
    ...
    def btn_text_operation_accept_click(self, button):
        ...
        if self.text_operation == 1:
            ...
            if (self.new_object_shape == VideoObject.ShapeAlignedRectangle or
                self.new_object_shape == VideoObject.ShapeQuadrilateral):
                default_position = np.array([[10, 10], [110, 10], [110, 110], [10, 110]], dtype=np.float64)
            else:
